...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride):
        super(InvertedResidual, self).__init__()
        self.stride = stride
        assert stride in [1, 2]  # 确保步长合法

        oup_inc = oup // 2  # 输出通道数的一半

        if self.stride == 1:  # 步长为 1
            #assert inp == oup_inc
            # 第一个分支，1x1 卷积 + 3x3 深度可分离卷积 + 1x1 卷积
            self.banch2 = nn.Sequential(
                # pw
                nn.Conv3d(oup_inc, oup_inc, 1, 1, 0, bias=False),  # 1x1 卷积
                nn.BatchNorm3d(oup_inc),  # 批量归一化
                nn.ReLU(inplace=True),  # 激活函数
                # dw
                nn.Conv3d(oup_inc, oup_inc, 3, stride, 1, groups=oup_inc, bias=False),  # 3x3 深度可分离卷积
                nn.BatchNorm3d(oup_inc),  # 批量归一化
                # pw-linear
                nn.Conv3d(oup_inc, oup_inc, 1, 1, 0, bias=False),  # 1x1 卷积
                nn.BatchNorm3d(oup_inc),  # 批量归一化
            ...
            # 第一个分支，3x3 深度可分离卷积 + 1x1 卷积
            self.banch1 = nn.Sequential(
                # dw
                nn.Conv3d(inp, inp, 3, stride, 1, groups=inp, bias=False),  # 3x3 深度可分离卷积
                nn.BatchNorm3d(inp),  # 批量归一化
                # pw-linear
                nn.Conv3d(inp, oup_inc, 1, 1, 0, bias=False),  # 1x1 卷积
                nn.BatchNorm3d(oup_inc),  # 批量归一化
                nn.ReLU(inplace=True),  # 激活函数
            )

            # 第二个分支，1x1 卷积 + 3x3 深度可分离卷积 + 1x1 卷积
            self.banch2 = nn.Sequential(
                # pw
                nn.Conv3d(inp, oup_inc, 1, 1, 0, bias=False),  # 1x1 卷积
                nn.BatchNorm3d(oup_inc),  # 批量归一化
